...
REFERENCES `Cancoes`(`song_id`);

ALTER TABLE `SpotifyClone`.`Seguindo`
ADD FOREIGN KEY (`artist_id`)
REFERENCES `Artista`(`artist_id`);
...
ALTER TABLE `SpotifyClone`.`Cancoes`
...
ALTER TABLE `SpotifyClone`.`Usuario`
ADD FOREIGN KEY (`plan_id`)
REFERENCES `Planos`(`plan_id`);


INSERT INTO `SpotifyClone`.`Usuario` (`user_id`, `username`, `age`, `plan_date`)
...
INSERT INTO `SpotifyClone`.`Planos` (`plan_id`, `plan_type`, `plan_value`) 
VALUES (1, 'gratuito', 0),
  (2, 'universitário', 5.99),
  (3, 'pessoal', 6.99),
  (4, 'familiar', 7.99);

INSERT INTO `SpotifyClone`.`Artista` (`artist_id`, `first_name`, `last_name`) 
...
INSERT INTO `SpotifyClone`.`Cancoes` (`song_id`, `song_name`, `duration`, `artist_id`, `album_id`)
...
INSERT INTO `SpotifyClone`.`Historico` (`user_id`, `song_id`, `reproduction_date`)
